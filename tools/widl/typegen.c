                                ...
                                const char *prefix, unsigned int proc_offset )
{
    ...
    if ((target.cpu == CPU_i386 || target.cpu == CPU_WASM32) && args)
    ...
    else if (target.cpu != CPU_i386 && target.cpu != CPU_WASM32)
    {
    ...
    if (has_ret)
    {
        ...
        fprintf( file, ")%s;\n",
                 (target.cpu != CPU_i386 && target.cpu != CPU_WASM32) ? "_RetVal.Simple" : "*(LONG_PTR *)&_RetVal" );
    }
    print_file( file, 0, "}\n\n");
}
